import { App, Editor, MarkdownView, Menu, Notice, Plugin, WorkspaceLeaf } from 'obsidian';
import { GyazoPluginSettings, DEFAULT_SETTINGS, GyazoImage } from './src/types/index';
...
export default class GyazoPlugin extends Plugin {
    ...
    async onload() {
        ...
        const ribbonIconEl = this.addRibbonIcon('camera', this.getTranslation().ribbonTooltip, async () => {
        ...
        this.registerObsidianProtocolHandler('gyazo-oauth', async (params) => {
            if (params.code) {
                try {
                    ...
                    new Notice(this.getTranslation().loginButton + ' ' + this.getTranslation().save);
        ...
        this.registerDomEvent(document, 'drop', async (evt: DragEvent) => {
            ...
            if (evt.dataTransfer?.getData('gyazo/image')) {
                ...
                if (image) {
                    if (image.type === 'mp4') {
                        editor.replaceSelection(`<video src="${image.url}" controls></video>`);
                    } else {
    ...
    async getGyazoImages(): Promise<GyazoImage[]> {
        ...
        try {
            ...
            new Notice('Failed to fetch Gyazo images. Check your access token and try again.');
            return [];
        }
    }
    ...
    async activateView() {
        ...
        let leaf: WorkspaceLeaf | null = workspace.getLeavesOfType(GYAZO_VIEW_TYPE)[0];
